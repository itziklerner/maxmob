...
{
$node=array();
#print "SELECT `xref_max_pn` FROM `xref` WHERE `xref_comp_pn` = ".$_POST['xref_comp_pn']."';";
...
while ($prod = mysql_fetch_row($products, MYSQL_BOTH))
				 ...
				 $sql = sprintf('SELECT ds.`qv_pk` , ds.`qv_parts_short_text` , ds.`qv_en_title` , p.`qv_part_no`
FROM `part_number`.`qv_datasheets` AS ds, `part_number`.`qv_parts` AS p
WHERE ds.`qv_pk` = p.`qv_datasheets_fk`
AND p.`qv_part_no`
IN ( %s )', $str);
